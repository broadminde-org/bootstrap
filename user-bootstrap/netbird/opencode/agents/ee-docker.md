...
- DIR_LISTING: For directory listing, use the `read` tool with the directory path.
  Never call `bash ls` — it will be denied by the permission profile.
  Only bash commands matching `docker *` are permitted.
...
1. INVENTORY: read existing Dockerfiles and Compose files
   Use `read` tool (not bash) to list directories. Example:
   read(filePath="/home/stack/netbird-docker/ansible-runner/") not bash("ls -la /home/stack/...")
...
5. CONTEXT: build context = repo root (this stack uses `45-build/run.sh` to build the caddy-custom image)
