...
install_node() {
  # nvmrc is informational only; EE_NODE_VERSION is the source of truth.
  local nvmrc_version
  nvmrc_version=$(cat "$(dirname "$0")/../../../.nvmrc" 2>/dev/null || true)
  if [[ -n "$nvmrc_version" && "$nvmrc_version" != "$EE_NODE_VERSION" ]]; then
    echo "  NOTE: .nvmrc pins '${nvmrc_version}' but EE_NODE_VERSION='${EE_NODE_VERSION}' -- using EE_NODE_VERSION."
  fi
