...
if ! require_cmd sqlite3; then
  log_skip "sqlite3 not installed — cannot vacuum"
  exit 0
fi
...
log_info "vacuuming $KILO_DB (${size_before})"
run_cmd sqlite3 "$KILO_DB" VACUUM 2>/dev/null || {
  log_err "VACUUM failed — database may be locked"
  exit 1
}

# WAL checkpoint cleanup
run_cmd sqlite3 "$KILO_DB" "PRAGMA wal_checkpoint(TRUNCATE);" 2>/dev/null || true

size_after=$(du -h "$KILO_DB" 2>/dev/null | awk '{print $1}')
log_ok "vacuumed Kilo DB: ${size_before} → ${size_after}"