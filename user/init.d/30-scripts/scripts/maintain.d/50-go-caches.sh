...
# `go env` runs via user_run so `sudo maintain` reports/mutates the deploy
# user's cache, not root's (root's GOCACHE is a different directory).
before="$(human_size "$(user_run go env GOCACHE 2>/dev/null)")"
log_info "GOCACHE before: $before"
user_run go clean -cache
after="$(human_size "$(user_run go env GOCACHE 2>/dev/null)")"
...
for dir in "${CACHE_DIRS[@]}"; do
  ...
  if [[ "${DRY_RUN:-0}" == "1" ]]; then
    ...
    log_err "failed to remove $name cache"
    step_failed=1
  fi
done
