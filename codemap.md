...
| `54-crowdsec` | dir | Registers CrowdSec apt repo via `install.crowdsec.net`; installs crowdsec + crowdsec-firewall-bouncer-iptables; installs crowdsecurity/sshd and crowdsecurity/caddy collections; manages Caddy log source as managed `acquis.d/caddy-central.yaml` drop-in (not acquis.yaml append); manages LAPI `listen_uri` 0.0.0.0:8080 for docker-bridge reachability; stages ufw 172.16.0.0/12 → 8080/tcp rule; enables/starts both services | Apt repo script overwrites existing sources; `cscli collections install` skips already-installed; compare-before-write/skip throughout, ufw rule skipped when present; restart on config change to a running daemon |
...
| `58-mdns` | dir | Patches nsswitch.conf with `mdns4_minimal`; writes `/etc/hosts` 127.0.1.1 line with FQDN (domain from `/etc/resolv.conf` `domain`/`search`); generates `/etc/avahi/avahi-daemon.conf` with private-address interfaces only (RFC1918/CGNAT/ULA; public uplinks excluded) and `use-ipv6=yes`; restarts avahi-daemon | nsswitch.conf sed is idempotent; hosts line skipped if already correct; avahi-daemon.conf compared bytewise before write; avahi-daemon restarted each run |
