...
- PLAN_CHECK: Before starting any multi-step task, check if a `_plans/` file exists whose title or content matches the task topic. If found, read its "what NOT to repeat" / "constraints" / "known failures" section first. Treat its listed anti-patterns as hard constraints — not suggestions. Failing to check an existing plan and then repeating a documented mistake is a SESSION_SPLIT_SIGNAL.
